...
const STORAGE_KEY_LOADS     = 'gpt_loads_v1';
...
/**
 * Carrega o objeto de cargas salvas.
 * Estrutura: { [categoryId]: { [exerciseIndex]: string } }
 * @returns {Object}
 */
function loadLoads() {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEY_LOADS)) || {};
  } catch {
    return {};
  }
}
...
/**
 * Salva a carga de um exercício específico.
 * @param {string} categoryId
 * @param {number} exerciseIdx
 * @param {string} value
 */
function saveLoad(categoryId, exerciseIdx, value) {
  const loads = loadLoads();
  if (!loads[categoryId]) loads[categoryId] = {};
  if (value.trim() === '') {
    delete loads[categoryId][exerciseIdx];
  } else {
    loads[categoryId][exerciseIdx] = value.trim();
  }
...
function renderWorkout(category) {
  ...
  const loads = loadLoads();
  const categoryLoads = loads[category.id] || {};

  list.innerHTML = '';
  category.exercises.forEach((ex, idx) => {
    const savedLoad = categoryLoads[idx] || '';
    const item = createExerciseItem(ex, idx, done.has(idx), savedLoad, category.id, (checked) => {
 ...
 * @param {string} savedLoad  - Valor de carga salvo anteriormente
 * @param {string} categoryId - Id da categoria pai
 * @param {function(boolean): void} onToggle
 * @returns {HTMLLIElement}
 */
function createExerciseItem(exercise, idx, isDone, savedLoad, categoryId, onToggle) {
  const li = document.createElement('li');
  li.className = `exercise-item${isDone ? ' done' : ''}`;
  li.setAttribute('aria-label', `${exercise.name}: ${isDone ? 'concluído' : 'pendente'}`);

  li.innerHTML = `
    <div class="exercise-check" aria-hidden="true" role="button" tabindex="0">
    ...
    <div class="exercise-content">
      ...
      <div class="exercise-load-row">
        <label class="load-label" for="load-${categoryId}-${idx}">⚖️ Carga</label>
        <input
          class="load-input"
          id="load-${categoryId}-${idx}"
          type="text"
          inputmode="decimal"
          placeholder="ex: 20kg, 2×15kg"
          maxlength="30"
          value="${savedLoad}"
          aria-label="Última carga usada em ${exercise.name}"
        />
      </div>
    </div>
  `;

  // Toggle apenas no checkbox e no nome — NÃO no input
  const checkEl = li.querySelector('.exercise-check');
  const contentEl = li.querySelector('.exercise-content');
  const inputEl = li.querySelector('.load-input');
  ...
  // Clique no check
  checkEl.addEventListener('click', (e) => { e.stopPropagation(); toggle(); });
  checkEl.addEventListener('keydown', (e) => {
    if (e.key === ' ' || e.key === 'Enter') { e.preventDefault(); toggle(); }
  });

  // Clique no nome/sets/notes (mas não no input)
  contentEl.addEventListener('click', (e) => {
    if (e.target === inputEl || e.target.closest('.exercise-load-row')) return;
    toggle();
  });

  // Salva a carga ao sair do campo (blur) ou ao pressionar Enter
  inputEl.addEventListener('blur', () => saveLoad(categoryId, idx, inputEl.value));
  inputEl.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); inputEl.blur(); showToast('Carga salva ✓'); }
  });
  // Previne que clique no input propague para o li
